...
void viewingAngle::paint(QPainter *painter, const QStyleOptionGraphicsItem*, QWidget*){
    ...
    painter->drawLine(0, 0, 120 * cos(qDegreesToRadians(mAngle/2)), 120 * sin(qDegreesToRadians(mAngle/2)) );
    painter->drawLine(0, 0, 120 * cos(qDegreesToRadians(mAngle/2)), -120 * sin(qDegreesToRadians(mAngle/2)));
}

QRectF viewingAngle::boundingRect() const {
//    return QRectF(QPointF(0, -120 *  sin(qDegreesToRadians(mAngle/2))),
//                  QPointF(120 * cos(qDegreesToRadians(mAngle/2)), 120 * sin(qDegreesToRadians(mAngle/2))));
     return QRectF(0, -120, 120, 2*120);
}
